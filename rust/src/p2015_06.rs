...
impl LightGrid {
    ...
    pub fn toggle_rect(&mut self, upper_left: (usize, usize), lower_right: (usize, usize)) {
        self.for_light_rect(upper_left, lower_right, |light_status| {
            *light_status = usize::from(*light_status == 0);
        });
    }
    ...
    fn for_light_rect<Cb: Fn(&mut usize)>(
        &mut self,
...
fn parse_instruction_point(instruction_point_str: &str) -> (usize, usize) {
    ...
    (
        coordinate_strings[0].parse::<usize>().unwrap(),
        coordinate_strings[1].parse::<usize>().unwrap(),
    )
}
...
fn parse_instruction_rect(instruction_rect_str: &str) -> ((usize, usize), (usize, usize)) {
    ...
    (
        parse_instruction_point(point_strings[0]),
        parse_instruction_point(point_strings[1]),
    )
}
...
fn process_light_instructions<ToggleFn, OnFn, OffFn>(
    instruction_iter: &mut dyn Iterator<Item = &str>,
    mut toggle_fn: ToggleFn,
    mut on_fn: OnFn,
    mut off_fn: OffFn,
) where
...
fn solve(input: &str, _log_fn: Option<fn(&str)>) -> (String, String) {
    ...
    // Part 1: Find the total brightness of the resulting light grid after processing and applying each instruction using the part 1 rules.
    let shared_light_grid1: std::rc::Rc<std::cell::RefCell<LightGrid>> =
        std::rc::Rc::new(std::cell::RefCell::new(LightGrid::new(1000)));
    process_light_instructions(
        &mut input.lines(),
        |ul, lr| {
            shared_light_grid1.borrow_mut().toggle_rect(ul, lr);
        },
        |ul, lr| {
            shared_light_grid1.borrow_mut().turn_on_rect(ul, lr);
        },
        |ul, lr| {
            shared_light_grid1.borrow_mut().turn_off_rect(ul, lr);
        },
    );
    let part1_result = shared_light_grid1.borrow().get_total_brightness().to_string();

    // Part 2: Find the total brightness of the resulting light grid after processing and applying each instruction using the part 2 rules.
    let shared_light_grid2: std::rc::Rc<std::cell::RefCell<LightGrid>> =
        std::rc::Rc::new(std::cell::RefCell::new(LightGrid::new(1000)));
    process_light_instructions(
        &mut input.lines(),
        |ul, lr| {
            shared_light_grid2.borrow_mut().inc_rect(ul, lr, 2);
        },
        |ul, lr| {
            shared_light_grid2.borrow_mut().inc_rect(ul, lr, 1);
        },
        |ul, lr| {
            shared_light_grid2.borrow_mut().dec_rect(ul, lr);
        },
    );
    let part2_result = shared_light_grid2.borrow().get_total_brightness().to_string();

    (part1_result, part2_result)
}

#[linkme::distributed_slice(crate::SOLUTIONS)]
static SOLUTION: crate::Solution = crate::Solution::new(2015, 6, solve);
