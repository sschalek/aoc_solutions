...
fn md5_hash_key_and_num(key_bytes: &[u8], num: usize) -> [u8; 16] {
    // Create a stack-based buffer that can hold the string representation of the given number,
    // and write the string representation of the number to it.
    let mut num_buffer = itoa::Buffer::new();
    let num_string = num_buffer.format(num);

    let mut hasher = md5::Md5::new();
    hasher.update(key_bytes);
    hasher.update(num_string);
...
fn find_first_zero_prefixed_hash_number(key: &str, leading_zero_count: usize) -> usize {
    ...
    loop {
        ...
        let chunk_result = (chunk_begin..chunk_end)
            .into_par_iter()
            .map(|n| (n, md5_hash_key_and_num(key_bytes, n)))
