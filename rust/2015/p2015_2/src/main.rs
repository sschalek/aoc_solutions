...
fn input_lines() -> impl Iterator<Item = String> {
    ...
    reader.lines().map(std::result::Result::unwrap)
}
...
fn package_dimensions() -> impl Iterator<Item = (i32, i32, i32)> {
    ...
    let dimensions = input_lines().map(|l| {
        l.split('x')
...
fn get_paper_area_required(dimensions: (i32, i32, i32)) -> i32 {
    ...
    let mut side_areas = [dimensions.0 * dimensions.1,
    ...
    side_areas
        .iter()
        .fold(side_areas[0], |total, side| total + 2 * side)
}
...
fn get_ribbon_length_required(dimensions: (i32, i32, i32)) -> i32 {
    ...
    let side_perimeters = [(dimensions.0 + dimensions.1) * 2,
...
fn main() {
    ...
    let total_paper_area = package_dimensions().fold(0, |total, dimensions| {
        total + get_paper_area_required(dimensions)
    });
    println!("{total_paper_area}");

    // Part 2: Print out the total length of ribbon required for all packages specified in the input.
    let total_ribbon_length = package_dimensions().fold(0, |total, dimensions| {
        total + get_ribbon_length_required(dimensions)
    });
